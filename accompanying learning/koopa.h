...
/// 全局变量定义列表和函数定义列表Value Fun Block Type
...
/// Type的基本类型 Array Pointer Func i32 void
...
// 中文部分为samashu注释。
///以下四函数分别从不同位置获得文本形式的Koopa IR并解析为内存形式。
...
///以下四函数将内存形式的Koopa IR转化为llvm IR，且为文本形式
...
/// 利用builder和program创建raw program（不可手动修改）
